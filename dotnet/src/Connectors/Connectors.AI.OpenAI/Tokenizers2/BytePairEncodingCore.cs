...
public class BytePairEncodingCore
{
    ...
    private readonly Dictionary<int, byte[]> _specialTokensDecoder;
    private readonly Regex _regexTls;
    private readonly Regex _specialTokenPatternRegex;
    ...
    )
    {
        ...
        this._encoder = bytePairEncoder == null
            ? new Dictionary<byte[], int>(comparer)
            : new Dictionary<byte[], int>(bytePairEncoder, comparer);
        this._decoder = bytePairEncoder?.ToDictionary(pair => pair.Value, pair => pair.Key.ToArray())
                        ?? new Dictionary<int, byte[]>();

        this._specialTokensEncoder = specialTokenEncoder ?? new Dictionary<string, int>();
        this._specialTokensDecoder =
            specialTokenEncoder?.ToDictionary(pair => pair.Value, pair => Encoding.UTF8.GetBytes(pair.Key))
            ?? new Dictionary<int, byte[]>();
        this._regexTls = tokenPatternRegex ?? new Regex("");

        var parts = this._specialTokensEncoder.Keys.Select(Regex.Escape).ToArray();
        var joinedParts = string.Join("|", parts);
        try
        {
            this._specialTokenPatternRegex = new Regex(joinedParts);
        }
    ...
    public (List<int>, int) EncodeNative(string text, ISet<string> allowedSpecial)
    {
        ...
        while (true)
        {
            var nextSpecialStartIndex =
                FindNextSpecialStartIndex(text, allowedSpecial, startIndex, this._specialTokenPatternRegex);

            var endIndex = nextSpecialStartIndex ?? text.Length;
            var textSegment = text.Substring(startIndex, endIndex - startIndex);

            foreach (var match in this._regexTls.Matches(textSegment).Cast<Match>())
            {
                var encodedPiece = Encoding.UTF8.GetBytes(match.Value);
                if (this._encoder.TryGetValue(encodedPiece, out var token))
                {
                    lastTokenLength = 1;
                ...
                var tokens = BytePairEncode(encodedPiece, this._encoder).ToList();
            ...
            if (nextSpecialStartIndex.HasValue)
            {
                ...
                var specialTokenValue = this._specialTokensEncoder[specialToken];
    ...
    public List<byte> DecodeNative(int[] tokens)
    {
        ...
        {
            if (!this.TryDecodeToken(token, out var tokenBytes))
            {
                continue;
            }
    ...
    {
        return this._decoder.TryGetValue(token, out tokenBytes) ||
               this._specialTokensDecoder.TryGetValue(token, out tokenBytes);
    }
